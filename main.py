...
def _shopify_fallback_search(user_text: str, origin: str, limit: int = 100) -> tuple[list, list]:
    ...
    for p in products:
        if (
            ...
            or text in (p["body_html"].lower())
            or text in (p["sku"].lower())
        ):
            results.append(p)
...
def chat():
    ...
    try:
        ...
        attempts = []
        if not productos:
            print("[DEBUG] Sin resultados por keywords; usando fallback literal multi-estrategia...")
        ...
        if not productos:
            ...
            return jsonify({
                ...
                "meta": {
                    ...
                    "diagnostic": attempts  # útil mientras ajustamos catálogo/filtros
                }
            })
