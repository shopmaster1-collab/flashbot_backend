  ...
  function renderProductCards(products, isNewSearch) {
    ...
    products.forEach((p) => {
      ...
      const inv = Array.isArray(p.inventory) && p.inventory.length
        ...
        : '';
      const compatibility = p.compatibility && typeof p.compatibility === 'object' ? p.compatibility : {};
      const reasons = Array.isArray(compatibility.reasons) ? compatibility.reasons.filter(Boolean).slice(0, 3) : [];
      const warnings = Array.isArray(compatibility.warnings) ? compatibility.warnings.filter(Boolean).slice(0, 2) : [];
      const specsSummary = Array.isArray(compatibility.specs_summary) ? compatibility.specs_summary.filter(Boolean).slice(0, 3) : [];
      const matchBadges = reasons.length
        ? `<div class="mf-product-match">${reasons.map((r) => `<span>${escapeHtml(r)}</span>`).join('')}</div>`
        : '';
      const specs = specsSummary.length
        ? `<p class="mf-product-specs">${specsSummary.map((r) => escapeHtml(r)).join(' · ')}</p>`
        : '';
      const warn = warnings.length
        ? `<p class="mf-product-warn">${warnings.map((r) => escapeHtml(r)).join(' · ')}</p>`
        : '';
      card.innerHTML = `
